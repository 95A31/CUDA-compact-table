...
namespace Fca
{
    ...
    class Matrix
    {
        ...
        public:
            ...
            __host__ __device__ inline get(u32 rIdx, u32 cIdx);
            __host__ __device__ inline set(u32 rIdx, u32 cIdx, T const & value);
            __host__ __device__ inline static void set(u32 rows, u32 cols, u32 * data, u32 rIdx, u32 cIdx, bool value);
            __host__ __device__ inline static T const * const get(u32 rows, u32 cols, u32 const * data, u32 rIdx, u32 cIdx);
            __host__ __device__ inline static u32 getDataSize(u32 rows, u32 cols);
            __host__ __device__ inline static void print(u32 rows, u32 cols, T const * data, char endl = '\n');
    };
    ...
    void Matrix<T>::set(u32 rows, u32 cols, T * data, u32 rIdx, u32 cIdx, T const & value)
    {
        assert(rIdx < rows);
        assert(cIdx < cols);
        data[rIdx * cols + cIdx] = value;
    }
